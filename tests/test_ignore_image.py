...
class TestIgnoreImage(TestBase):
    """Test of attributes 'ignore_image'"""
...
# hello`[SQUARE]` and `(ROUND)`
"""
    def common_ignore_image_codeblock_default(self, toc_txt):
        ...
        self.assert_In('- hello`[SQUARE]` and `(ROUND)`', toc_txt)
    def test_ignore_image_codeblock_default(self):
        toc_txt = self.commonSetup(self.ignore_image_codeblock_text.format(''))
        self.common_ignore_image_codeblock_default(toc_txt)
    def test_ignore_image_codeblock_true(self):
        toc_txt = self.commonSetup(self.ignore_image_codeblock_text.format('ignore_image="true"'))
        self.common_ignore_image_codeblock_default(toc_txt)
    def test_ignore_image_codeblock_false(self):
        toc_txt = self.commonSetup(self.ignore_image_codeblock_text.format('ignore_image="false"'))
        self.assert_In('- ![IMAGE](image.png) and LINK and \[SQUARE\] and \(ROUND\)', toc_txt)
        self.assert_In('- hello![IMAGE](image.png) and LINK and \[SQUARE\] and \(ROUND\)', toc_txt)
        self.assert_In('- `![IMAGE](image.png) and LINK and [SQUARE] and (ROUND)`', toc_txt)
        self.assert_In('- hello`![IMAGE](image.png) and LINK and [SQUARE] and (ROUND)`', toc_txt)
